...
export default function LogPage() {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<ActivityCategory>("transport");
  ...
  useEffect(() => {
    if (
      subtypes.length > 0 &&
      !subtypes.find((s) => s.id === selectedSubtype)
    ) {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      setValue(
        String(
          subtypes.find((s) => s.id === selectedSubtype)?.defaultValue || "",
        ),
      );
      setNote("");
      setShowSuccess(true);
  ...
  const currentSubtype = ACTIVITY_SUBTYPES.find(
      ...
      <div className="grid grid-2" style={{ marginBottom: 24 }}>
        ...
        <div className="card animate-in">
          ...
          <form onSubmit={handleSubmit}>
                      ...
                      style={
                        selectedCategory === cat
                          ? {
                              ...
                              boxShadow: `0 0 12px ${CATEGORY_CONFIG[cat].color}40`,
                            }
                          : {}
                      }
                    >
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="subtype-select">
                Activity Type
              </label>
              <select
                id="subtype-select"
              ...
              >
                {subtypes.map((s) => (
                  <option key={s.id} value={s.id}>
                    {s.icon} {s.label} ({s.emissionFactor} kg CO₂/{s.unit})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="grid grid-2">
              <div className="form-group">
                <label className="form-label" htmlFor="value-input">
                  Amount ({currentSubtype?.unit || "unit"})
                </label>
                <input
                  id="value-input"
                  type="number"
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ color: cat.color, marginRight: 8 }}
                    >
                    ...
                    <button
                      ...
                      aria-label={`Delete activity ${act.subtypeId}`}
                    >
                      🗑️
                    </button>
                  </div>
